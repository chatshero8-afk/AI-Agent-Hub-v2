...
import { Sun, Moon, LayoutGrid, BarChart3, Menu, X, Rocket, LogOut, User, Shield, LayoutTemplate } from 'lucide-react';
...
interface LayoutProps {
  ...
  activeTab: 'hub' | 'analytics' | 'profile' | 'settings' | 'widgets';
  onTabChange: (tab: 'hub' | 'analytics' | 'profile' | 'settings' | 'widgets') => void;
}
...
export default function Layout({ children, activeTab, onTabChange }: LayoutProps) {
  ...
  const navItems: Array<{ id: 'hub' | 'analytics' | 'profile' | 'settings' | 'widgets', label: string, icon: any, path: string }> = [
  ...
  const handleNav = (id: 'hub' | 'analytics' | 'profile' | 'settings' | 'widgets', path: string) => {
    onTabChange(id);
    navigate(path);
  };
  ...
  return (
    <div className="min-h-screen bg-grid flex flex-col">
          ...
          <div className="flex h-16 items-center justify-between">
            <div className="flex items-center gap-3 group">
              ...
              <span className="text-lg font-black tracking-tight uppercase italic drop-shadow-[0_0_10px_rgba(255,255,255,0.4)] group-hover:drop-shadow-[0_0_15px_rgba(255,255,255,0.8)] transition-all">
                <span className="shimmer-text text-slate-800 dark:text-white">Chats</span><span className="text-primary shimmer-text drop-shadow-[0_0_8px_rgba(139,92,246,0.8)]">Hero</span>
              </span>
            </div>
      ...
      <main className={cn("mx-auto py-8", activeTab === 'profile' ? "max-w-full px-12" : "max-w-7xl px-4 sm:px-6 lg:px-8")}>
        {children}
      </main>

      <footer className="mt-auto py-8 border-t border-zinc-200 dark:border-zinc-800">
        <div className={cn("mx-auto text-center text-zinc-500 text-xs font-medium tracking-widest uppercase", activeTab === 'profile' ? "max-w-full px-12" : "max-w-7xl px-4")}>
