      ...
      <div className="flex justify-end items-center mb-8 gap-3 flex-wrap relative">
        ...
        {showDatePicker && (
          <>
            ...
            <div className="absolute top-12 right-12 z-50 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-2xl shadow-xl w-[480px] p-0 flex flex-col md:flex-row overflow-hidden">
              
              {/* Presets Sidebar */}
              <div className="w-full md:w-36 bg-slate-50 dark:bg-slate-800/50 p-2 border-b md:border-b-0 md:border-r border-slate-200 dark:border-white/10 flex flex-row md:flex-col overflow-x-auto gap-1">
                <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest p-2">Recently Used</div>
                {['Today', 'Yesterday', 'Last 7 days', 'Last 14 days', 'Last 30 days', 'This month', 'Last month', 'This year'].map(preset => (
                  <button 
                    key={preset}
                    onClick={() => setPreset(preset)}
                    className={\`text-left px-3 py-2 rounded-lg text-xs font-medium whitespace-nowrap transition-colors \${activePreset === preset ? 'bg-primary/10 text-primary dark:text-purple-400 font-bold' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-200/50 dark:hover:bg-slate-700/50'}\`}
                  >
                    {preset}
                  </button>
                ))}
